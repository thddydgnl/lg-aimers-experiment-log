#!/usr/bin/env python3
"""Immutable source gate for the expanded-selector fine-pitch MoE."""
...
PREREG = ROOT / "experiments/params/v5_expanded_fine_pitch_moe_preregister.json"
REPORT = RESULTS / "v5_expanded_fine_pitch_moe_source_gate.json"
YEARS = (2020, 2021)
STAGES = {year: f"v5_expanded_fine_pitch_moe_source{year}" for year in YEARS}
...
def main() -> None:
    ...
    for year in YEARS:
        ...
        semantic["pass"] = bool(
            ...
            and semantic["auto_label_coverage_all"]
        ...
        folds[year] = {
            ...
            "semantic": semantic,
            "paths": {"candidate": candidate_path, "parent": parent_path, "metadata": metadata_path},
        }
        ...
        for gamma in prereg["source_protocol"]["top_level_blend_grid"]:
            ...
            for year in YEARS:
                ...
                years[str(year)] = evaluate(
                    ...
                    3910000 + 10000 * year + int(float(gamma) * 100),
                )
                cache[(year, float(gamma))] = np.clip(
                    fold["parent"] + float(gamma)
                    * (fold["routed_raw"] - fold["parent"]), 1e-6, 1.0 - 1e-6
                )
    ...
    if selected is not None:
        for year in YEARS:
            fold = folds[year]
            output = PRED / f"v5_expanded_fine_pitch_moe_source_{year}.npz"
            if output.exists():
