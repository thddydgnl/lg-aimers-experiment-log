...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--validation-prior-mode",
        choices=("history", "recent3", "recent2", "r_recent3", "r_recent2_f_recent"),
        default="history",
        help="Validation-season prior rule; selection must come from a separate pseudo-forward run.",
    )
...
def run_fold(frame: pd.DataFrame, validation_season: int, args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.validation_prior_mode != "history":
        from experiments.run_e15_pseudo_forward import candidate_priors

        validation_prior = float(
            candidate_priors(history, validation_season)[args.validation_prior_mode]
        )
...
def main() -> None:
    ...
    payload = {
        "metadata": {
            ...
            "validation_prior_mode": args.validation_prior_mode,
