#!/usr/bin/env python3
"""Immutable 2020/2021 source gate for within-game pairwise ranking."""
...
PREREG = ROOT / "experiments/params/v5_game_pairwise_rank_preregister.json"
MODEL_PARAMS = ROOT / "experiments/params/v5_game_pairwise_rank_model.json"
ENGINE = ROOT / "experiments/run_v2_rolling.py"
REPORT = ROOT / "experiments/results/v5_game_pairwise_rank_source_gate.json"
KEY = "catboost_game_pairwise_rank"
...
def main() -> None:
    ...
    for year in YEARS:
        ...
        rank_path = PRED / f"v5_game_pairwise_rank_source_{year}.npz"
        parent_artifact = load(parent_path)
        rank_artifact = load(rank_path)
        for key in ("y", "row_index", "cluster"):
            if not np.array_equal(parent_artifact[key], rank_artifact[key]):
        ...
        folds[year] = {
            ...
            "deviation": rank_artifact[KEY].astype(np.float64) - 0.5,
        }
        input_hashes[str(year)] = {
            "parent": digest(parent_path),
            "ranker": digest(rank_path),
        }
    ...
    for offset, year in enumerate(YEARS):
        ...
        selected_metrics[str(year)] = route_metrics(
            ...
            fold["cluster"],
            regular,
            seed=8227000 + 10000 * offset,
        )
        output = PRED / f"v5_game_pairwise_rank_selected_source_{year}.npz"
        if output.exists():
        ...
        np.savez_compressed(
            ...
            rank_deviation=fold["deviation"].astype(np.float32),
