#!/usr/bin/env python3
"""Select or confirm the preregistered multinomial context-offset model."""
...
SELECTION = ROOT / "experiments/results/v5_multinomial_offset_selection.json"
CONFIRMATION = ROOT / "experiments/results/v5_multinomial_offset_confirmation.json"
...
def baseline_prediction(
    ...
    for align_key in ("y", "row_index", "cluster"):
        if not np.array_equal(candidate[align_key], artifact[align_key]):
            raise ValueError(f"alignment mismatch {baseline_name}/{year}/{align_key}")
...
def evaluate(
    ...
    prediction[route] += gamma * (
        candidate["multinomial_offset"][route].astype(np.float64) - baseline[route]
    )
...
def select(args: argparse.Namespace, prereg: dict, all_types: np.ndarray) -> None:
    ...
    stem = prereg["fixed_recipe"]["development_stem"]
    candidates = {
        year: load(PRED / f"{stem}_{year}.npz") for year in (2022, 2023)
    }
    trials: list[dict] = []
        ...
        for baseline_index, baseline_name in enumerate(BASELINES):
            ...
            for year in (2022, 2023):
                ...
                result = evaluate(
                    ...
                    float(gamma),
                    args.bootstrap,
                    830000 + 1000 * baseline_index + year + int(100 * gamma),
                )
    ...
    selected = (
        sorted(eligible, key=lambda trial: (-trial["minimum_R_gain"], trial["gamma"]))[0]
        if eligible
        else None
    )
    report = {
...
def confirm(args: argparse.Namespace, prereg: dict, all_types: np.ndarray) -> None:
    ...
    candidate = load(
        PRED / f"{prereg['fixed_recipe']['confirmation_stem']}_2024.npz"
    )
    ...
    np.savez_compressed(
        PRED / "v5_multinomial_offset_locked_2024.npz",
        y=candidate["y"],
        row_index=candidate["row_index"],
        cluster=candidate["cluster"],
        multinomial_offset=candidate["multinomial_offset"],
