#!/usr/bin/env python3
"""Test the historically selected nested-deviation direction on V4."""
...
SOURCE_REPORT = ROOT / "experiments/results/v4_nested_deviations.json"
BASE_REPORT = ROOT / "experiments/results/v4_robust_group_stack.json"
REPORT = ROOT / "experiments/results/v4_nested_direction_stack.json"
...
def main() -> None:
    ...
    selected_weights = spec["selected_weights"]["weights"]

    frames, artifacts = load_frames()
    add_columns(frames, artifacts)
    stacks = {
        year: load_npz(PRED / f"v4_robust_group_stack_{year}.npz")
        for year in (2023, 2024)
    }
    anchor_scalar = float(
        base_report["candidates"]["base_plus_r_anchor_from_r"]["selected_scalar"]
    )
    base = {
        2023: (
            stacks[2023]["base"].astype(np.float64)
            + anchor_scalar * stacks[2023]["direction_r_anchor_from_r"]
        ),
        2024: stacks[2024]["base_plus_r_anchor_from_r"].astype(np.float64),
    }
    ...
    for year, source_years in ((2023, (2021, 2022)), (2024, (2022, 2023))):
        mask, library = transition_library(
            frames, artifacts, source_years, year, route
        )
        axis_directions[year] = {}
        correction = np.zeros(int(mask.sum()), dtype=np.float64)
        for axis in AXES:
            values = library[axis.name][float(selected_k[axis.name])]
            weighted = float(selected_weights[axis.name]) * values
            axis_full = np.zeros(len(mask), dtype=np.float64)
            axis_full[mask] = weighted
            axis_directions[year][axis.name] = axis_full
            correction += weighted
    ...
    p23 = base[2023] + scalar * directions[2023]
    p24 = base[2024] + scalar * directions[2024]
    s23 = raw_score(y[2023], p23)
    s24 = raw_score(y[2024], p24)

    matrix23 = np.column_stack([axis_directions[2023][axis.name] for axis in AXES])
    matrix24 = np.column_stack([axis_directions[2024][axis.name] for axis in AXES])
    ...
    candidates = {
        "base_plus_nested_fixed_direction": {
            "selected_scalar": scalar,
            "selection_gain": s23 - base_scores[2023],
            "confirmation_gain": s24 - base_scores[2024],
            "confirmation_score": s24,
            "expected_lb_median": s24 + MEDIAN_OFFSET,
        },
        "base_plus_nested_axes_joint": {
            "coefficients": {
                axis.name: float(value) for axis, value in zip(AXES, coefficients)
            },
    ...
    output23 = PRED / "v4_nested_direction_stack_2023.npz"
    output24 = PRED / "v4_nested_direction_stack_2024.npz"
    np.savez_compressed(
        output23,
        y=y[2023], row_index=stacks[2023]["row_index"], cluster=stacks[2023]["cluster"],
        base=base[2023], direction_nested=directions[2023],
    ...
    np.savez_compressed(
        ...
        base=base[2024], direction_nested=directions[2024],
        base_plus_nested_fixed_direction=np.clip(p24, 0.0, 1.0),
        base_plus_nested_axes_joint=np.clip(joint24, 0.0, 1.0),
