#!/usr/bin/env python3
"""Immutable source gate for conditional historical-physics integration."""
...
PREREG = (
    ROOT
    / "experiments/params/v5_conditional_physics_integration_preregister.json"
)
SIGNAL_REPORT = RESULTS / "v5_conditional_physics_integration_signal_source.json"
REPORT = RESULTS / "v5_conditional_physics_integration_source.json"
...
def main() -> None:
    ...
    if signal_report["status"] != prereg["semantic_gate"][
        ...
        raise ValueError("conditional integration signal source did not pass")
    ...
    for year in YEARS:
        signal_path = RESULTS / f"v5_conditional_physics_integration_signal_{year}.npz"
        ...
        for gamma in prereg["gamma_grid"]:
            ...
            for year in YEARS:
                ...
                if direction_name == "physics_delta":
                    direction = fold["parent"] + fold["signals"][direction_name]
                else:
                    direction = fold["signals"][direction_name]
                result = evaluate(
                    fold["artifact"],
                    fold["parent"],
                    direction,
                    ...
                    gamma,
                    bootstrap,
                    6370000 + 100000 * direction_index + 10000 * year
                    + int(gamma * 100),
                )
    ...
    for year in YEARS:
        fold = folds[year]
        output = PRED / f"v5_conditional_physics_integration_source_{year}.npz"
        if output.exists():
