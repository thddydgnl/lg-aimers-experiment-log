#!/usr/bin/env python3
"""Immutable source gate for the fine-else-coarse joint pitch model."""
...
PREREG = ROOT / "experiments/params/v5_hier_pitch_joint_preregister.json"
REPORT = RESULTS / "v5_hier_pitch_joint_source_gate.json"
YEARS = (2020, 2021)
STAGES = {year: f"v5_hier_pitch_joint_source{year}" for year in YEARS}
KEY = "catboost_hier_pitch_joint"
EXPECTED_FINE = {
    "ChangeUp", "Curveball", "Cutter", "Fastball", "Other", "Sinker",
    "Slider", "Splitter",
}
EXPECTED_COARSE = {"breaking", "fastball", "offspeed"}
...
def main() -> None:
    ...
    for year in YEARS:
        ...
        latent = {value.split("|pitch=", 1)[1] for value in classes}
        fine = {value.removeprefix("fine=") for value in latent if value.startswith("fine=")}
        coarse = {
            value.removeprefix("coarse=")
            for value in latent
            if value.startswith("coarse=")
        }
        semantic = {
            "fine_label_coverage_all": float(details["fine_label_coverage_all"]),
            "coarse_label_coverage_all": float(
                details["coarse_label_coverage_all"]
            ),
            "hierarchical_label_coverage_all": float(
                details["hierarchical_label_coverage_all"]
            ),
            "hierarchical_label_coverage_R": float(
                details["hierarchical_label_coverage_R"]
            ),
            ...
            "observed_fine_types": sorted(fine),
            "observed_coarse_groups": sorted(coarse),
        ...
        semantic["pass"] = bool(
            semantic["hierarchical_label_coverage_all"]
            >= float(
                prereg["semantic_gate"][
                    "minimum_hierarchical_label_coverage_each_year"
                ]
            )
            and semantic["fine_label_coverage_all"]
            >= float(
                prereg["semantic_gate"][
                    "minimum_fine_label_coverage_each_year"
                ]
            )
            and fine == EXPECTED_FINE
            and coarse == EXPECTED_COARSE
        ...
        for gamma in prereg["source_protocol"]["top_level_blend_grid"]:
            ...
            for year in YEARS:
                ...
                metrics[str(year)] = evaluate(
                    ...
                    2910000 + 10000 * year + int(float(gamma) * 100),
                )
    ...
    if selected is not None:
        for year in YEARS:
            fold = folds[year]
            output = PRED / f"v5_hier_pitch_joint_source_{year}.npz"
            if output.exists():
            ...
            np.savez_compressed(
                ...
                hier_pitch_joint_raw=fold["candidate"][KEY].astype(np.float64),
    ...
    print(json.dumps(safe({"status": report["status"], "semantic": report["semantic"], "selected": selected}), ensure_ascii=False, indent=2))
