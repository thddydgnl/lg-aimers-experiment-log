#!/usr/bin/env python3
"""Immutable source gate for the auto-pitch joint outcome model."""
...
PREREG = ROOT / "experiments/params/v5_auto_pitch_joint_preregister.json"
REPORT = RESULTS / "v5_auto_pitch_joint_source_gate.json"
YEARS = (2020, 2021)
STAGES = {year: f"v5_auto_pitch_joint_source{year}" for year in YEARS}
...
def main() -> None:
    ...
    for year in YEARS:
        ...
        for alignment_key in ("y", "row_index", "cluster"):
            if not np.array_equal(
                candidate[alignment_key], parent_artifact[alignment_key]
            ):
        ...
        semantic = {
            ...
            "auto_probability_features": details["auto_probability_features"],
        ...
        semantic["pass"] = bool(
            ...
            >= float(
                prereg["semantic_gate"][
                    "minimum_history_auto_label_coverage_each_year"
                ]
            )
            ...
            and len(semantic["auto_probability_features"]) == 10
        ...
        for gamma in prereg["source_protocol"]["top_level_blend_grid"]:
            ...
            for year in YEARS:
                ...
                metrics[str(year)] = evaluate(
                    ...
                    3010000 + 10000 * year + int(float(gamma) * 100),
                )
            ...
            full_gains = [
                metrics[str(y)]["routes"]["full"]["gain"] for y in YEARS
            ]
            trials.append(
                {
                    "gamma": float(gamma),
    ...
    if selected is not None:
        for year in YEARS:
            fold = folds[year]
            output = PRED / f"v5_auto_pitch_joint_source_{year}.npz"
            if output.exists():
            ...
            np.savez_compressed(
                ...
                auto_pitch_joint_raw=fold["candidate"][KEY].astype(np.float64),
