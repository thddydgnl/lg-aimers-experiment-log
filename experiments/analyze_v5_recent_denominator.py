#!/usr/bin/env python3
"""Select and confirm the preregistered recent-denominator C direction."""
...
from experiments.run_v5_h1_residual import (
...
PREREG = ROOT / "experiments/params/v5_recent_denominator_preregister.json"
SELECTION = ROOT / "experiments/results/v5_recent_denominator_selection.json"
CONFIRMATION = ROOT / "experiments/results/v5_recent_denominator_confirmation.json"
ANCHOR_STEM = "v3_sparse_m3_frozen"
DEV_STEM = "v5_recent_denominator_c_dev2223"
CONFIRM_STEM = "v5_recent_denominator_c_confirm24"
...
def evaluate(
    ...
    score_metrics = metrics(
        fold["y"], fold["anchor"], prediction, game_type
    )
    interval_r = cluster_bootstrap_score_gain(
        fold["y"],
        fold["anchor"],
        prediction,
        fold["cluster"],
    ...
    interval_all = cluster_bootstrap_score_gain(
        fold["y"],
        fold["anchor"],
        prediction,
        fold["cluster"],
        np.ones(len(game_type), dtype=bool),
        bootstrap,
        seed + 10000,
    )
    return {
        "metrics": score_metrics,
        "bootstrap_R": interval_r,
        "bootstrap_all": interval_all,
    }
...
def run_selection(args: argparse.Namespace, prereg: dict, all_types: np.ndarray) -> None:
    ...
    folds = {year: load_fold(year, DEV_STEM) for year in (2022, 2023)}
    trials = []
    for gamma in prereg["selection"]["gamma_grid"]:
        per_fold = {
            str(year): evaluate(
                folds[year],
                game_types(folds[year], all_types),
                float(gamma),
                args.bootstrap,
                74000 + year + int(round(float(gamma) * 100)),
            )
        ...
        eligible = bool(
            float(gamma) > 0.0
            and all(gain > 0.0 for gain in r_gains)
            and all(
    ...
    if eligible:
        selected = sorted(
            eligible, key=lambda trial: (-trial["min_R_gain"], trial["gamma"])
        )[0]
    report = {
        ...
        "trials": trials,
        "selected": selected,
        "status": "locked" if selected is not None else "failed_no_eligible_gamma",
    }
    ...
    if selected is not None:
        ...
        for year in (2022, 2023):
            ...
            np.savez_compressed(
                PRED / f"v5_recent_denominator_locked_{year}.npz",
                y=fold["y"],
                row_index=fold["row_index"],
                cluster=fold["cluster"],
                anchor=fold["anchor"],
                denominator_c=fold["candidate"],
...
def run_confirmation(
    ...
    fold = load_fold(2024, CONFIRM_STEM)
    result = evaluate(
        fold,
        game_types(fold, all_types),
        gamma,
        args.bootstrap,
        95024,
    )
    ...
    np.savez_compressed(
        PRED / "v5_recent_denominator_locked_2024.npz",
        y=fold["y"],
        row_index=fold["row_index"],
        cluster=fold["cluster"],
        anchor=fold["anchor"],
    ...
    development_full_gains = [
        selection["selected"]["folds"][str(year)]["metrics"]["all"]["gain"]
        for year in (2022, 2023)
    ]
