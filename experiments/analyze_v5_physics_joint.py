#!/usr/bin/env python3
"""Immutable 2020/2021 source gate for the physics-cluster joint model."""
...
PREREG = ROOT / "experiments/params/v5_physics_joint_preregister.json"
REPORT = RESULTS / "v5_physics_joint_source_gate.json"
YEARS = (2020, 2021)
STAGES = {year: f"v5_physics_joint_source{year}" for year in YEARS}
KEY = "catboost_physics_joint"
...
def main() -> None:
    ...
    for year in YEARS:
        ...
        observed_clusters = sorted(
            {value.split("|physics=", 1)[1] for value in classes}
        )
        success_classes = [
            value for value in classes if value.startswith("success|")
        ]
        semantic = {
            "physics_label_coverage_all": float(
                details["physics_label_coverage_all"]
            ),
            "physics_label_coverage_R": float(
                details["physics_label_coverage_R"]
            ),
            ...
            "configured_cluster_count": int(details["cluster_count"]),
            "observed_clusters": observed_clusters,
            "cluster_counts": details["cluster_counts"],
            "physics_columns": details["physics_columns"],
            "fine_probability_features": details["fine_probability_features"],
            "current_pitch_physics_used_at_inference": bool(
                details["current_pitch_physics_used_at_inference"]
            ),
        ...
        semantic["pass"] = bool(
            semantic["physics_label_coverage_all"]
            >= float(gate["minimum_history_physics_label_coverage_each_year"])
            and semantic["configured_cluster_count"]
            == int(gate["required_cluster_count_each_year"])
            and len(observed_clusters)
            == int(gate["required_cluster_count_each_year"])
            and semantic["success_joint_class_count"]
            >= int(gate["minimum_success_joint_classes_each_year"])
            and len(semantic["fine_probability_features"]) == 10
            and not semantic["current_pitch_physics_used_at_inference"]
        ...
        for gamma in prereg["source_protocol"]["top_level_blend_grid"]:
            ...
            for year in YEARS:
                ...
                metrics[str(year)] = evaluate(
                    ...
                    2810000 + 10000 * year + int(float(gamma) * 100),
                )
    ...
    if selected is not None:
        for year in YEARS:
            fold = folds[year]
            output = PRED / f"v5_physics_joint_source_{year}.npz"
            if output.exists():
            ...
            np.savez_compressed(
                ...
                physics_joint_raw=fold["candidate"][KEY].astype(np.float64),
